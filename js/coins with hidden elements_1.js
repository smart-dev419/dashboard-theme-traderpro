      ...
      response.json().then((data) => {
        ...
        for (let i = 0; i < coins.length; i++) {
          ...
          els[0].innerHTML +=
            ...
            data.coins[i].symbol +
            'USDT"></span><span style="background: blue; color: BLACK;" id="' +
            data.coins[i].symbol +
            'USDT">' +
            Math.trunc(parseFloat(data.coins[i].quote.USD.price).toFixed(8)) +
            ' </span><span style="display: none" id="o' +
            data.coins[i].symbol +
            'USDTb"></span><span style="color: BLACK;" id="' +
            data.coins[i].symbol +
            'USDTb">' +
            parseFloat(data.coins[i].quote.USD.price).toFixed(8) +
            " </span></li>";
          els[1].innerHTML +=
            ...
            parseFloat(data.coins[i].quote.USD.price).toFixed(8) +
            "</span>" +
            '<span style="display: block; background: yellow" id="o' +
            data.coins[i].symbol +
            '-PERPb"></span><span   id="' +
            data.coins[i].symbol +
            '-PERPb">' +
          ...
          els[3].innerHTML +=
            '<li><span style="display: none" id="o' +
            data.coins[i].symbol +
            'USDTh"></span><span style="color: BLACK;" id="' +
            data.coins[i].symbol +
            'USDTh">' +
            parseFloat(data.coins[i].quote.USD.price).toFixed(8) +
            ' </span><span style="display: none" id="o' +
            data.coins[i].symbol +
            'USDThb"></span><span style="color: BLACK;" id="' +
            data.coins[i].symbol +
            'USDThb">' +
            parseFloat(data.coins[i].quote.USD.price).toFixed(8) +
            " </span></li>";
          ...
          wsbn.onmessage = (event) => {
            ...
            let priceB = parseFloat(object.a).toFixed(8);
            let priceBb = parseFloat(object.b).toFixed(8);
...
function onMessageFTX(event) {
  ...
  if (payload) {
    if (payload.channel === "ticker" && payload.type === "update") {
      price = parseFloat(payload.data.ask).toFixed(8);
      priceb = parseFloat(payload.data.bid).toFixed(8);
      id = payload.market;
...
function onMessagecoinbase(event) {
  try {
    ...
    if (payload.best_ask) {
      ...
      pricecb = parseFloat(payload.best_ask).toFixed(8);
      pricecbb = parseFloat(payload.best_bid).toFixed(8);
...
function onMessagekraken(event) {
  try {
    ...
    if (payload[1].a[0]) {
      idK = payload[3];
      priceK = parseFloat(payload[1].a[0]).toFixed(8);
      priceKb = parseFloat(payload[1].b[0]).toFixed(8);
...
function onMessagebitmex(event) {
  try {
    ...
    if ((payloadB.action = "update" && payloadB.data[0].askPrice != null)) {
      priceBit = parseFloat(payloadB.data[0].askPrice).toFixed(8);
      priceBitb = parseFloat(payloadB.data[0].bidPrice).toFixed(8);
...
function onMessagehuobi(evt) {
  ...
  reader.onload = function (e) {
    ...
    if (msg.search("ping") > 0) {
      ...
      priceBh = parseFloat(datah.tick.asks[0]).toFixed(8);

      priceBhb = parseFloat(datah.tick.bids[0]).toFixed(8);
